...
public class MoveUtils {
	...
	public static void strafe(float var0) {
		if (MovementUtils.isMoving()) {
			double var1 = getPlayerDirection();
			mc.thePlayer.motionX = -Math.sin(var1) * (double) var0;
			mc.thePlayer.motionZ = Math.cos(var1) * (double) var0;
		}
	}
	...
	public static boolean isOverVoid() {
		for (double posY = mc.thePlayer.posY; posY > 0.0D; posY--) {
			if (!(mc.theWorld.getBlockState(new BlockPos(mc.thePlayer.posX, posY, mc.thePlayer.posZ))
	...
	public static double calculateFriction(final double moveSpeed, final double lastDist,
		...
		final double materialFriction = mc.thePlayer.isInWater() ? 0.8899999856948853
				: (mc.thePlayer.isInLava() ? 0.5350000262260437 : 0.9800000190734863);
	...
	public static void setMotion(EventMove e, double speed, float yaw) {
		...
		if ((forward == 0.0D) && (strafe == 0.0D)) {
			...
			mc.thePlayer.motionX = e.x = forward * speed * Math.cos(Math.toRadians(yaw + 90.0F))
					+ strafe * speed * Math.sin(Math.toRadians(yaw + 90.0F));
			mc.thePlayer.motionZ = e.z = forward * speed * Math.sin(Math.toRadians(yaw + 90.0F))
	...
	public static Block getBlockAtPosC(double x, double y, double z) {
		...
		return mc.theWorld.getBlockState(new BlockPos(inPlayer.posX + x, inPlayer.posY + y, inPlayer.posZ + z))
				.getBlock();
	}
