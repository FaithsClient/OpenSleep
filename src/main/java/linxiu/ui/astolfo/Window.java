...
public class Window {
	...
	public void render(int mouseX, int mouseY) {
		...
		} else if (this.category.name().equals("Other")) {
			staticColor = new Color(38, 154, 255).getRGB();
		}	

		// GlStateManager.pushMatrix();
		if (expand > 0) {
		//	 RenderUtil.rectangleBordered(this.x - 0.5, this.y - 0.5, this.x +
		//	 90.5, this.y + 1.5 + expand, 1.0f,
		//	 staticColor, staticColor);// category
		//	 RenderUtil.rectangleBordered(this.x, this.y, this.x + 90, this.y
		//	 + 1.0 + expand, 1.0f,
		//	 new Color(0, 5, 0,180).getRGB(), new
		//	 Color(0,5,0,180).getRGB());// category
		}
		RenderUtil.drawGradientRect(this.x, this.y + 4, this.x + 101, this.y + 17,
		...
		// FontLoaders.kiona17.drawStringWithShadow(this.category.name().toLowerCase(),
		// this.x + 3, this.y + 6, new Color(HUD.r.getValue().intValue(),
		// HUD.g.getValue().intValue(), HUD.b.getValue().intValue()).getRGB());
		ScaledResolution res = new ScaledResolution(Minecraft.getMinecraft());
		if (this.category.name().equals("Combat")) {
			FontLoaders.TahomaBold16.drawStringWithShadow("blatant", this.x + 3, this.y + 8,
					new Color(255,255,255)
							.getRGB());
			// FontLoaders.NovICON18.drawString("D", this.x + 80 , this.y + 7,
			// -1);
		}
		if (this.category.name().equals("Render")) {
			FontLoaders.TahomaBold16.drawStringWithShadow("visual", this.x + 3, this.y + 8,
					new Color(255,255,255)
							.getRGB());
			// FontLoaders.NovICON18.drawString("C", this.x + 79, this.y + 7,
			// -1);
		}
		if (this.category.name().equals("Movement")) {
			FontLoaders.TahomaBold16.drawStringWithShadow("movement", this.x + 3, this.y + 8,
					new Color(255,255,255)
							.getRGB());
			// FontLoaders.NovICON18.drawString("A", this.x + 80, this.y + 7,
			// -1);
		}
		if (this.category.name().equals("Player")) {
			FontLoaders.TahomaBold16.drawStringWithShadow("player", this.x + 3, this.y + 8,
					new Color(255,255,255)
							.getRGB());
			// FontLoaders.NovICON18.drawString("B", this.x + 80, this.y +
			// 7,-1);
		}
		if (this.category.name().equals("Legit")) {
			FontLoaders.TahomaBold16.drawStringWithShadow("legit", this.x + 3, this.y + 8,
					new Color(255,255,255)
							.getRGB());
			// FontLoaders.NovICON18.drawString("F", this.x + 80, this.y +
			// 7,-1);
		}
		/*
		 * GlStateManager.pushMatrix(); GlStateManager.translate(this.x + 90 -
		 * 10, this.y + 5, 0.0f); GlStateManager.rotate((float) this.angel,
		 * 0.0f, 0.0f, -1.0f); GlStateManager.translate(-this.x + 90 - 10,
		 * -this.y + 5, 0.0f); GlStateManager.popMatrix();
		 */
		if (this.expand > 0) {
